...
//MARK: Assertions an Preconditions(İddialar ve Ön Koşullar)
//İddialar ve Preconditions, çalışma zamanında gerçekleşen kontrolleridir. Bunları, daha fazla kod yürütmeden önce temel bir koşulun yerine getirildiğinden emin olmak için kullanırsınız. Assertions an Preconditions'daki Boolean koşulu doğru olarak değerlendirilirse, kod yürütme normal şekilde devam eder. Koşul yanlış olarak değerlendirilirse, programın mevcut durumu geçersizdir; kod yürütme sona erer ve uygulamanız sonlandırılır.

//Assertions an Preconditions, kodlama sırasında yaptığınız varsayımları ve beklentilerinizi ifade etmek için kullanırsınız, böylece bunları kodunuzun bir parçası olarak dahil edebilirsiniz. Assertions, geliştirme sırasında hataları ve yanlış varsayımları bulmanıza yardımcı olur ve Preconditions, üretimde sorunları tespit etmenize yardımcı olur.

//Assertions an Preconditions, çalışma zamanında beklentilerinizi doğrulamanın yanı sıra, kod içinde yararlı bir belgeleme biçimi haline de gelir. Yukarıdaki Hata İşleme bölümünde tartışılan hata koşullarından farklı olarak, Assertions an Preconditions'lar kurtarılabilir veya beklenen hatalar için kullanılmaz. Başarısız bir Assertions an Preconditions, geçersiz bir program durumunu gösterdiğinden, başarısız bir assertion'ı yakalamanın bir yolu yoktur. Geçersiz bir durumdan kurtarma imkansızdır. Bir onay başarısız olduğunda, programın en az bir veri parçası geçersizdir, ancak neden geçersiz olduğunu veya başka bir durumun da geçersiz olup olmadığını bilemezsiniz.
